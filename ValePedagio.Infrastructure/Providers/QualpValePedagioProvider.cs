...
public sealed class QualpValePedagioProvider : IValePedagioProvider
{
    private readonly IValePedagioProviderConfigurationRepository _repo;
    private readonly QualpHttpClient _client;

    public QualpValePedagioProvider(IValePedagioProviderConfigurationRepository repo, QualpHttpClient client)
    {
        _repo = repo;
        _client = client;
    }
    ...
    public ValePedagioProviderDescriptor Descriptor =>
        ValePedagioProviderCatalog.Descriptors.Single(d => d.Type == ValePedagioProviderType.QualP);
    ...
    {
        var config = await _repo.GetAsync(tenantId, ValePedagioProviderType.QualP, ct);
        ...
        return new RestProviderSettings(
            ...
            RestProviderSettings.G(c, "providerDocument") ?? ValePedagioProviderDocuments.Documents[ValePedagioProviderType.QualP],
            RestProviderSettings.G(c, "documentType") ?? "Cupom",
            TimeSpan.FromSeconds(Math.Clamp(timeout, 5, 120)));
    }
}

public sealed class QualpHttpClient
{
    private const string ProviderName = "QualP";
    private readonly RestValePedagioHttpClient _rest;

    public QualpHttpClient(HttpClient http, ILogger<QualpHttpClient> logger)
