...
public sealed class TargetValePedagioProvider : IValePedagioProvider
{
    private readonly IValePedagioProviderConfigurationRepository _repo;
    private readonly TargetHttpClient _client;

    public TargetValePedagioProvider(IValePedagioProviderConfigurationRepository repo, TargetHttpClient client)
    {
        _repo = repo;
        _client = client;
    }
    ...
    public ValePedagioProviderDescriptor Descriptor =>
        ValePedagioProviderCatalog.Descriptors.Single(d => d.Type == ValePedagioProviderType.Target);
    ...
    {
        var config = await _repo.GetAsync(tenantId, ValePedagioProviderType.Target, ct);
        ...
        return new RestProviderSettings(
            ...
            RestProviderSettings.G(c, "providerDocument") ?? ValePedagioProviderDocuments.Documents[ValePedagioProviderType.Target],
            RestProviderSettings.G(c, "documentType") ?? "Cupom",
            TimeSpan.FromSeconds(Math.Clamp(timeout, 5, 120)));
    }
}

// Target delivers receipts as base64 in the callback/sync response and supports callback webhook.
public sealed class TargetHttpClient
{
    private const string ProviderName = "Target";
    private readonly RestValePedagioHttpClient _rest;

    public TargetHttpClient(HttpClient http, ILogger<TargetHttpClient> logger)
