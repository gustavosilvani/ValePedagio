...
public sealed class AmbiparValePedagioProvider : IValePedagioProvider
{
    private readonly IValePedagioProviderConfigurationRepository _repo;
    private readonly AmbiparHttpClient _client;

    public AmbiparValePedagioProvider(IValePedagioProviderConfigurationRepository repo, AmbiparHttpClient client)
    {
        _repo = repo;
        _client = client;
    }
    ...
    public ValePedagioProviderDescriptor Descriptor =>
        ValePedagioProviderCatalog.Descriptors.Single(d => d.Type == ValePedagioProviderType.Ambipar);
    ...
    {
        var config = await _repo.GetAsync(tenantId, ValePedagioProviderType.Ambipar, ct);
        ...
        return new RestProviderSettings(
            ...
            RestProviderSettings.G(c, "providerDocument") ?? ValePedagioProviderDocuments.Documents[ValePedagioProviderType.Ambipar],
            RestProviderSettings.G(c, "documentType") ?? "Cupom",
            TimeSpan.FromSeconds(Math.Clamp(timeout, 5, 120)));
    }
}

// Ambipar uses polling (no callback). Bearer auth.
public sealed class AmbiparHttpClient
{
    private const string ProviderName = "Ambipar";
    private readonly RestValePedagioHttpClient _rest;

    public AmbiparHttpClient(HttpClient http, ILogger<AmbiparHttpClient> logger)
