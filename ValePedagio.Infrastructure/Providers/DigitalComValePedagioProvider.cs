...
public sealed class DigitalComValePedagioProvider : IValePedagioProvider
{
    private readonly IValePedagioProviderConfigurationRepository _repo;
    private readonly DigitalComHttpClient _client;

    public DigitalComValePedagioProvider(IValePedagioProviderConfigurationRepository repo, DigitalComHttpClient client)
    {
        _repo = repo;
        _client = client;
    }
    ...
    public ValePedagioProviderDescriptor Descriptor =>
        ValePedagioProviderCatalog.Descriptors.Single(d => d.Type == ValePedagioProviderType.DigitalCom);
    ...
    {
        var config = await _repo.GetAsync(tenantId, ValePedagioProviderType.DigitalCom, ct);
        ...
        return new RestProviderSettings(
            ...
            RestProviderSettings.G(c, "providerDocument") ?? ValePedagioProviderDocuments.Documents[ValePedagioProviderType.DigitalCom],
            RestProviderSettings.G(c, "documentType") ?? "Cupom",
            TimeSpan.FromSeconds(Math.Clamp(timeout, 5, 120)));
    }
}

// DigitalCom supports callback: Purchase returns EmProcessamento; confirmation arrives via webhook
// at POST /api/v1/vale-pedagio/provedores/DigitalCom/callbacks (handled by ProviderCallbacksController).
public sealed class DigitalComHttpClient
{
    private const string ProviderName = "DigitalCom";
    private readonly RestValePedagioHttpClient _rest;

    public DigitalComHttpClient(HttpClient http, ILogger<DigitalComHttpClient> logger)
