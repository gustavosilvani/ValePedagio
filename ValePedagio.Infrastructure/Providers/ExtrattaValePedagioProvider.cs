...
public sealed class ExtrattaValePedagioProvider : IValePedagioProvider
{
    private readonly IValePedagioProviderConfigurationRepository _repo;
    private readonly ExtrattaHttpClient _client;

    public ExtrattaValePedagioProvider(IValePedagioProviderConfigurationRepository repo, ExtrattaHttpClient client)
    {
        _repo = repo;
        _client = client;
    }
    ...
    public ValePedagioProviderDescriptor Descriptor =>
        ValePedagioProviderCatalog.Descriptors.Single(d => d.Type == ValePedagioProviderType.Extratta);
    ...
    {
        var config = await _repo.GetAsync(tenantId, ValePedagioProviderType.Extratta, ct);
        ...
        return new RestProviderSettings(
            ...
            RestProviderSettings.G(c, "providerDocument") ?? ValePedagioProviderDocuments.Documents[ValePedagioProviderType.Extratta],
...
// Extratta uses retry for transient failures and polling for status updates.
public sealed class ExtrattaHttpClient
{
    private const string ProviderName = "Extratta";
    private readonly RestValePedagioHttpClient _rest;

    public ExtrattaHttpClient(HttpClient http, ILogger<ExtrattaHttpClient> logger)
