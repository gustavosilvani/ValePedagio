...
public sealed class PamcardValePedagioProvider : IValePedagioProvider
{
    private readonly IValePedagioProviderConfigurationRepository _repo;
    private readonly PamcardHttpClient _client;

    public PamcardValePedagioProvider(IValePedagioProviderConfigurationRepository repo, PamcardHttpClient client)
    {
        _repo = repo;
        _client = client;
    }
    ...
    public ValePedagioProviderDescriptor Descriptor =>
        ValePedagioProviderCatalog.Descriptors.Single(d => d.Type == ValePedagioProviderType.Pamcard);
    ...
    {
        var config = await _repo.GetAsync(tenantId, ValePedagioProviderType.Pamcard, ct);
        ...
        return new RestProviderSettings(
            ...
            RestProviderSettings.G(c, "providerDocument") ?? ValePedagioProviderDocuments.Documents[ValePedagioProviderType.Pamcard],
            RestProviderSettings.G(c, "documentType") ?? "Cupom",
            TimeSpan.FromSeconds(Math.Clamp(timeout, 5, 120)));
    }
}

// Pamcard supports callback webhook: Purchase returns EmProcessamento; confirmation arrives via
// POST /api/v1/vale-pedagio/provedores/Pamcard/callbacks (ProviderCallbacksController).
public sealed class PamcardHttpClient
{
    private const string ProviderName = "Pamcard";
    private readonly RestValePedagioHttpClient _rest;

    public PamcardHttpClient(HttpClient http, ILogger<PamcardHttpClient> logger)
